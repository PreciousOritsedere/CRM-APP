...
import TableSampleLeads from '@/components/Leads/TableSampleLeads.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Source', title: 'source'},
          {label: 'Score', title: 'score', number: 'true'},

          {label: 'Contact', title: 'contact'},{label: 'Owner', title: 'owner'},];
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    leadsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Leads'
    : 'http://localhost:8080/api-docs/#/Leads';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Leads</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for leads</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleLeads checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
