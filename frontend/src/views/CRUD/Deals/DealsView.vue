...
import TableSampleDeals from '@/components/Deals/TableSampleDeals.vue'
...
const filters = ref([]);
const filtersList = [{label: 'DealName', title: 'name'},

          {label: 'Amount', title: 'amount', number: 'true'},
          {label: 'Lead', title: 'lead'},{label: 'Owner', title: 'owner'},];
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    dealsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Deals'
    : 'http://localhost:8080/api-docs/#/Deals';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Deals</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for deals</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleDeals checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
