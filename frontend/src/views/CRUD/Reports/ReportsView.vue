...
import TableSampleReports from '@/components/Reports/TableSampleReports.vue'
...
const filters = ref([]);
const filtersList = [{label: 'ReportName', title: 'name'},{label: 'Description', title: 'description'},{label: 'Type', title: 'type'},
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    reportsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Reports'
    : 'http://localhost:8080/api-docs/#/Reports';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Reports</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for reports</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleReports checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
