...
import TableSampleTags from '@/components/Tags/TableSampleTags.vue'
...
const filters = ref([]);
const filtersList = [{label: 'TagName', title: 'name'},
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    tagsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Tags'
    : 'http://localhost:8080/api-docs/#/Tags';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Tags</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for tags</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleTags checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
