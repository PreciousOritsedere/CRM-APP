...
import TableSampleEmail_campaigns from '@/components/Email_campaigns/TableSampleEmail_campaigns.vue'
...
const filters = ref([]);
const filtersList = [{label: 'CampaignName', title: 'name'},{label: 'Subject', title: 'subject'},{label: 'Content', title: 'content'},
...
const getEmail_campaignsCSV = async () => {
  const response = await axios({url: '/email_campaigns?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'email_campaignsCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('email_campaigns/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    email_campaignsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Email_campaigns'
    : 'http://localhost:8080/api-docs/#/Email_campaigns';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Email_campaigns</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
        label="New"
        to="/email_campaigns/new"
      />
      <BaseButton
        color="default"
        label="Add Filter"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for email_campaigns</a>

    <CardBox
      class="mb-6"
      title="Email_campaigns table"
      has-table
    >
      <TableSampleEmail_campaigns checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
