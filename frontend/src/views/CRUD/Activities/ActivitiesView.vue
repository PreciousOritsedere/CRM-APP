...
import TableSampleActivities from '@/components/Activities/TableSampleActivities.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Type', title: 'type'},{label: 'Notes', title: 'notes'},

          {label: 'User', title: 'user'},{label: 'Contact', title: 'contact'},];
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    activitiesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Activities'
    : 'http://localhost:8080/api-docs/#/Activities';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Activities</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for activities</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleActivities checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
