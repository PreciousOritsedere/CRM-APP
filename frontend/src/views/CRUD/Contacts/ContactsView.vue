...
import TableSampleContacts from '@/components/Contacts/TableSampleContacts.vue'
...
const filters = ref([]);
const filtersList = [{label: 'FirstName', title: 'first_name'},{label: 'LastName', title: 'last_name'},{label: 'Email', title: 'email'},{label: 'Phone', title: 'phone'},{label: 'Company', title: 'company'},{label: 'Industry', title: 'industry'},{label: 'Location', title: 'location'},
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    contactsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Contacts'
    : 'http://localhost:8080/api-docs/#/Contacts';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Contacts</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for contacts</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleContacts checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
