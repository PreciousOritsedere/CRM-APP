...
/* Delete item */
const deleteItem = () => {
  tagsStore.deleteItem(currentId.value)
}

const items = computed(() => tagsStore.data)

const headers = [

{ text: 'TagName', value: 'name'},]
  ...
  <table>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/tags/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
