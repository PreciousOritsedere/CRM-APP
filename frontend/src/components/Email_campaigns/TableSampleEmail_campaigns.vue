...
email_campaignsStore.fetch()

/* Delete item */
const deleteItem = () => {
  email_campaignsStore.deleteItem(currentId.value)
}

const items = computed(() => email_campaignsStore.data)

const headers = [

{ text: 'CampaignName', value: 'name'},
{ text: 'Subject', value: 'subject'},
{ text: 'Content', value: 'content'},
{ text: 'ScheduledDate', value: 'scheduled_date'},
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'content' && props.sortDirection]"
        @click="sort('content')"
      >Content</th>

      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'scheduled_date' && props.sortDirection]"
        @click="sort('scheduled_date')"
      >ScheduledDate</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/email_campaigns/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="scheduled_date">
                {{ dataFormatter.dateTimeFormatter(client.scheduled_date) }}
              </td>    
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          ...
          >
            <BaseButton
              ...
              :icon="mdiEye"
              small
              :to="`/email_campaigns/${client.id}/edit`"
            />
            <BaseButton
              color="danger"
              :icon="mdiTrashCan"
