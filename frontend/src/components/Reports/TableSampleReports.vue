...
/* Delete item */
const deleteItem = () => {
  reportsStore.deleteItem(currentId.value)
}

const items = computed(() => reportsStore.data)

const headers = [

{ text: 'ReportName', value: 'name'},
  ...
  <table>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/reports/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
