...
export const useReportsStore = defineStore('reports', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`reports${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/reports', { data: payload });
        this.showNotification('Reports has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/reports/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Reports has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
