...
export const useActivitiesStore = defineStore('activities', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`activities${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/activities', { data: payload });
        this.showNotification('Activities has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/activities/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Activities has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
